...
/// <summary>
/// The recovered-keys file has keys.json's shape so its entries paste straight across; these pin
/// that shape and the merge rules.
/// </summary>
public class RecoveredKeysTests : IDisposable
{
    ...
    public void RecordingAStemAgainMovesItRatherThanDuplicating()
    {
        ...
        // The old group emptied out, so only the new key remains, holding the stem once.
        var group = Assert.Single(List());
    ...
    {
        // Somebody pasted a key in as a string. It is not our group; the new key still lands.
